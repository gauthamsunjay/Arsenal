...
class MongoClientInterface {
    ...
    _handleMongo(cursor, filter, log, cb) {
        cursor.match(filter);
        const mainCursor = cursor.clone();
        return async.parallel({
            count: done => {
                const tmpCursor = mainCursor.clone();
                tmpCursor.project({ _id: 1 })
                .group({
                    _id: null,
                    count: { $sum: 1 },
                }).toArray((err, res) =>
                    this._handleCount(err, res && res[0] || {}, done));
            },
            data: done => {
                ...
                }).toArray((err, res) => this._handleEntries(err, res, done));
            },
            repData: done => {
                const tmpCursor = cursor.clone();
                tmpCursor.project({
            ...
            // free cursor for garbage collection
            mainCursor.close();
            const retResult = {
                count: res.count,
    ...
    getObjectMDStats(bucketName, bucketInfo, log, callback) {
        ...
        const aggregateCursor = c.aggregate().project({
            '_id': 1,
            'value.versionId': 1,
            'value.replicationInfo.backends': 1,
            'value.content-length': 1,
        ...
        async.parallel({
            version: done =>
                this._handleMongo(aggregateCursor, verFilter, log, done),
            null: done =>
                this._handleMongo(aggregateCursor, nullFilter, log, done),
            master: done =>
                this._handleMongo(aggregateCursor, mstFilter, log, done),
        }, (err, res) => {
            if (err) {
                return callback(err);
            }
            // free aggregateCursor for garbage collection
